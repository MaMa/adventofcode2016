...
const discs = input.split("\n").map(row => {
  const match = row.match(/Disc #(\d+) has (\d+) positions; at time=(\d+), it is at position (\d+)./)
  return {
    n: Number(match[1]),
    len: Number(match[2]),
    pos: Number(match[4])
...
function drop(time, discs) {
  for (disc of discs) {
    if ((time + disc.n + disc.pos) % disc.len) return false
  }
  return true
}

let time = 0;
while (!drop(time, discs)) time++
