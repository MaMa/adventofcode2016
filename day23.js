...
function tgl(inst) {
  let match
  if ((match = inst.match(/(tgl|dec) (\w)/))) {
    return `inc ${match[2]}`
  } else if ((match = inst.match(/inc (\w)/))) {
    return `dec ${match[1]}`
  } else if ((match = inst.match(/jnz (.+)$/))) {
    return `cpy ${match[1]}`
  } else if ((match = inst.match(/cpy (.+)$/))) {
    return `jnz ${match[1]}`
  }
  console.log('Invalid tgl', inst)
}
...
function execute(instuction, ptr, reg) {
  ...
  else if ((match = instruction.match(/jnz (\w) (\w)/))) {
    if (reg[match[1]] != 0) {
      ptr = ptr + Number(reg[match[2]]) - 1
    }
  }
  else if ((match = instruction.match(/tgl (\w)/))) {
    const idx = ptr + reg[match[1]] -1
    if (instructions[idx]) {
      instructions[idx] = tgl(instructions[idx])
    }
  }
  else {
