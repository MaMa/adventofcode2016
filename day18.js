...
function isSafe(row, pos) {
  return !(row[pos] && row[pos] == '^')
}

function isTrap(row, idx) {
  if (idx < 0 || idx >= row.length) return false
  return row.substring(idx, idx + 1) === TRAP
}

function findTrap(row, idx) {
  return (isTrap(row, idx - 1) && isTrap(row, idx) && !isTrap(row, idx + 1))
    || (!isTrap(row, idx - 1) && isTrap(row, idx) && isTrap(row, idx + 1))
    || (isTrap(row, idx - 1) && !isTrap(row, idx) && !isTrap(row, idx + 1))
    || (!isTrap(row, idx - 1) && !isTrap(row, idx) && isTrap(row, idx + 1))
}
...
function makeRow(previous) {
  ...
  for (let i = 0; i < previous.length; i++) {
    row += findTrap(previous, i) ? TRAP : SAFE;
  }
  return row
}
