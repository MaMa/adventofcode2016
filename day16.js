...
Your puzzle input is 00101000101111010.
*/

function dragon(input) {
  let reverse = ''
  for (let idx = input.length; idx > 0; idx--)
    reverse += input.slice(idx - 1, idx) == '1' ? '0' : '1'
...
function checksum(input) {
  ...
  for (let idx = 0; idx < input.length - 1; idx += 2) {
    out += checkPair(input.slice(idx, idx + 2))
  }
...
function part1(string, length) {
  ...
  return checksum(string.slice(0, length))
}
